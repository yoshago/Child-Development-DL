...
import os
import numpy as np
from src.Read_csv import data, folder_linux
...
for i in range(len(data)):
    for file in os.listdir(folder_linux + "/" + str(data.Dir[i])):
        if file.endswith(".avi"):
            if "depth" not in file:
                ...
                if name[-1] == 'a' or name[-1] == 'b':
                    name = name[:-1]
                all_data = VidToMatrix(os.path.join(folder_linux + "/" + str(data.Dir[i]), file), classification,
                                       int(data.format[i]), name)
